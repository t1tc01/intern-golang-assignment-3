...
func (lq *LocationQuery) QueryGeometries() *GeometryQuery {
	query := (&GeometryClient{config: lq.config}).Query()
...
func (lq *LocationQuery) Clone() *LocationQuery {
	...
	return &LocationQuery{
		...
		withGeometries: lq.withGeometries.Clone(),
...
func (lq *LocationQuery) WithGeometries(opts ...func(*GeometryQuery)) *LocationQuery {
	query := (&GeometryClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withGeometries = query
	return lq
}
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := lq.withGeometries; query != nil {
		if err := lq.loadGeometries(ctx, query, nodes,
			func(n *Location) { n.Edges.Geometries = []*Geometry{} },
			func(n *Location, e *Geometry) { n.Edges.Geometries = append(n.Edges.Geometries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LocationQuery) loadGeometries(ctx context.Context, query *GeometryQuery, nodes []*Location, init func(*Location), assign func(*Location, *Geometry)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(geometry.FieldLocationID)
	}
	query.Where(predicate.Geometry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.GeometriesColumn), fks...))
	}))
