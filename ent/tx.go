...
func (tx *Tx) init() {
	tx.Apireq = NewApireqClient(tx.config)
	tx.Credentials = NewCredentialsClient(tx.config)
	tx.Earthquake = NewEarthquakeClient(tx.config)
	tx.FeatureType = NewFeatureTypeClient(tx.config)
	tx.FtypeEarthquake = NewFtypeEarthquakeClient(tx.config)
	tx.Geometry = NewGeometryClient(tx.config)
	tx.Location = NewLocationClient(tx.config)
	tx.PasswordResetRequest = NewPasswordResetRequestClient(tx.config)
	tx.Report = NewReportClient(tx.config)
	tx.Session = NewSessionClient(tx.config)
	tx.Source = NewSourceClient(tx.config)
	tx.SourceEarthquake = NewSourceEarthquakeClient(tx.config)
...
// applies a query, for example: Apireq.QueryXXX(), the query will be executed
